...
export const getFirebaseErrorMessage = (error) => {
  ...
  if (error.message && typeof error.message === "string") {
    ...
    if (error.message && error.message.includes("Firebase: Error (auth/")) {
    ...
    if (error.message && error.message.startsWith("auth/")) {
      return (
        FIREBASE_ERROR_MESSAGES[error.message] ||
    ...
    if (error.message && typeof error.message === "string") {
      ...
      for (const keyword of messageKeywords) {
        if (lowerMessage.includes(keyword.toLowerCase())) {
          ...
          if (
            keyword.includes("invalid-credential") ||
            keyword.includes("invalid credential")
          ) {
  ...
  if (error.code) {
    return (
      FIREBASE_ERROR_MESSAGES[error.code] || FIREBASE_ERROR_MESSAGES.default
    );
  }
